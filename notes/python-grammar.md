...
# 언패킹: 여러 개의 값이 하나로 묶여 있는 상자를 풀어 헤쳐서 각각의 변수에 나누어 담는 것
points = [(10, 20), (30, 40), (50, 60)]

# x에는 첫 번째 숫자, y에는 두 번째 숫자가 들어감
for x, y in points:
    print(f"X좌표는 {x}, Y좌표는 {y}입니다.")
...
| **형태** | **동작 설명** |
| --- | --- |
| `str.split()` | 공백(스페이스, 탭, 줄바꿈) 기준으로 분할 |
| `str.split('X')` | 문자 `'X'`를 기준으로 분할, X는 어디에도 속하지 않음 |
| `str.split('X', N)` | 문자 `'X'` 기준으로 최대 N번만 분할, X는 어디에도 속하지 않음 |
- `'X'`가 `str`에 없다면 자기 자신 하나만 담긴 리스트 반환

### strip()
- 문자열에서 양쪽 끝의 불필요한 공백이나 특정 문자 제거할 때 사용
- `문자열.strip()`이 빈 문자열일 경우 `false` 반환

### 포맷 문자열
- 문자열 안에 변수나 계산 결과를 원하는 양식에 맞춰 깔끔하게 끼워 넣기 위해 사용하는 문자열
 
| **구분** | **문법 형식** | **예시 코드** | **출력 결과** | **주요 활용** |
| --- | --- | --- | --- | --- |
| **디버깅** | `{변수=}` | `f"{name=}"` | `name='Alice'` | 변수명과 값을 한 번에 출력해 값 확인 |
| **소수점 제한** | `{값:.Nf}` | `f"{3.14159:.2f}"` | `3.14` | 소수점 $N$번째 자리까지 반올림 표기 |
| **천 단위 쉼표** | `{값:,}` | `f"{12500000:,}"` | `12,500,000` | 금액나 수량 단위 가독성 개선 |
| **자릿수 0 채우기** | `{값:0Nd}` | `f"{7:04d}"` | `0007` | 자릿수를 고정하고 빈 공간을 `0`으로 채움 |
| **정렬 (오른쪽)** | `{값:>N}` | `f"{'hi':>5}"` | `'   hi'` | $N$칸 공간 확보 후 오른쪽 정렬 |
| **정렬 (가운데+채우기)** | `{값:C^N}` | `f"{'hi':*^7}"` | `'**hi***'` | N칸 중앙 정렬 후 여백을 특정 문자($C$)로 채움 |
| **진수 변환** | `{값:b}`, `{값:#x}` | `f"{10:b}"`, `f"{10:#x}"` | `1010`, `0xa` | 2진수(`b`), 16진수(`x`, `#x`는 접두사 포함) |
...
# 인덱스를 Key로 가지는 딕셔너리 생성
fruit_dict = dict(enumerate(fruits))
print(fruit_dict) # 출력: {0: '사과', 1: '바나나', 2: '포도'}
...
- Match 객체: 정규표현식으로 문장을 찾았을 때 어디서 무엇을 발견했는지에 대한 상세한 정보를 담고 있는 결과 상자
    ...
    line = "ABCDEF"
    match = re.search(r"CD", line)  # "CD"를 검색

    print(match.start())  # 2 ('C'의 시작 위치)
    print(match.end())    # 4 ('D' 바로 다음 위치)
    ```
- 위치 앵커
    | **기호** | **의미** | **예시 및 설명** |
    | --- | --- | --- |
    | **`^`** | 문자열 또는 줄의 시작 | `^http` → `http`로 시작하는 문자열<br> 주의: `^`를 대괄호 안에서 쓸 경우 부정의 이미가 됨 |
    | **`$`** | 문자열 또는 줄의 끝 | `pdf$` → `pdf`로 끝나는 문자열 |
- 문자 클래스 기호
    | **기호** | **의미** | **예시 (매칭되는 결과)** |
    | --- | --- | --- |
...
- 대괄호(문자 집합 및 범위)
    ...
    | **패턴** | **설명** | **예시 및 매칭** |
    | --- | --- | --- |
    | `[abc]` | `a`, `b`, `c` 중 아무 문자 1개 | `[abc]` → `a` (O), `d` (X) |
    | `[a-z]` | 소문자 `a`부터 `z`까지 | `[a-z]` → `k` |
    | `[A-Z]` | 대문자 `A`부터 `Z`까지 | `[A-Z]` → `K` |
    | `[0-9]` | 숫자 `0`부터 `9`까지 | `[0-9]` → `5` |
    | `[^abc]` | 대괄호 안의 `^`는 NOT(제외) 의미 (`a,b,c` 제외) | `[^abc]` → `d` (O), `a` (X) |
- 정규식 탐색

    | **종류** | **구문** | **의미** | **예시 정규식** | **대상 문자열** | **매칭 결과** |
    | --- | --- | --- | --- | --- | --- |
    | **부정 룩어헤드** | `A(?!B)` | A 뒤에 B가 없어야 함 | `apple(?!pie)` | `apple juice` | `apple` |
    | **긍정 룩어헤드** | `A(?=B)` | A 뒤에 B가 있어야 함 | `apple(?=pie)` | `apple pie` | `apple` |
    | **부정 룩비하인드** | `(?<!B)A` | A 앞에 B가 없어야 함 | `(?<!USD )100` | `KRW 100` | `100` |
    | **긍정 룩비하인드** | `(?<=B)A` | A 앞에 B가 있어야 함 | `(?<=USD )100` | `USD 100` | `100` |

```python
r',(?![^<]*>)'
```
- 쉼표(,)를 찾는다. 단, 그 쉼표 바로 뒤를 보았을 때 `<` 문자가 아닌 글자들이 0개 이상 이어지다가 `>` 문자를 만나는 형태가 오지 않아야 한다.
- `,`: 탐색할 문자(쉼표)
- `(?!....)`: 쉼표 바로 뒤에 해당 조건이 붙어있는지 확인하고 조건이 맞으면 매칭에서 제외
- `[^<]*`: `<`를 제외한 모든 문자 0개 이상
- `>`: 닫는 꺽쇠
### Path.pathlib
